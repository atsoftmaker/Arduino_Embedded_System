# Lab 26: ESP32 Gateway ดึงข้อมูลจาก Node ผ่าน RS485 (Modbus Master)

## Overview (ภาพรวม)
นี่คือการรวมร่างของทั้งหลักสูตร: ESP32 ทำหน้าที่เป็น **Gateway** — สะพานเชื่อมระหว่างโลก 2 ใบ
- **ฝั่งล่าง (Field):** พูด Modbus RTU ผ่าน RS485 ไปอ่านข้อมูลจาก Node (Arduino UNO ที่เป็น Modbus Slave จาก Lab 22)
- **ฝั่งบน (Enterprise):** เชื่อม Wi-Fi องค์กร (จาก Lab 24–12) เตรียมส่งข้อมูลขึ้น OPS Server (Lab 27)
...
**บัส RS485 เชื่อมไปยัง UNO Node (Lab 22):**
...
#define SLAVE_ID 1     // ตรงกับ UNO ใน Lab 22
...
void setup() {
  ...
  Serial.println("--- Lab 26: ESP32 Modbus Gateway ---");
}
...
## คำอธิบายโค้ด
- **preTransmission / postTransmission** เป็น callback ที่ ModbusMaster เรียกอัตโนมัติก่อน/หลังส่ง เพื่อสลับขา DE/RE ของ MAX485 — ไม่ต้องคุมเองเหมือน Lab 19
...
- ข้อมูลที่อ่านได้ในตัวแปร `sensor/counter` จะถูกส่งต่อขึ้น OPS Server ใน **Lab 27**
