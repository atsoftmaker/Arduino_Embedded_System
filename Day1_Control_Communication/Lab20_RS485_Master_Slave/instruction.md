# Lab 20: RS485 แบบ Master / Slave Command
...
แลปนี้ต่อยอดจาก Lab 19 โดยสร้าง **Protocol คำสั่งอย่างง่าย**:
- Master ส่ง `READ` → Slave ตอบค่าเซนเซอร์ (จำลองด้วย VR)
- Master ส่ง `LED:ON` / `LED:OFF` → Slave ควบคุม LED ตามคำสั่ง แล้วตอบ `ACK`

นี่คือพื้นฐานก่อนเข้าสู่ Modbus (Lab 22) ซึ่งเป็น Protocol Master/Slave มาตรฐานสากล

## Hardware Wiring (การต่อวงจร)
ใช้การต่อ MAX485 เหมือน Lab 19 ทุกประการ (DI=D3, RO=D2, DE+RE=D4, A↔A, B↔B, GND ร่วม)
...
- Protocol นี้ใช้ข้อความ (ASCII) อ่านง่ายตอนเรียน แต่ Modbus จะใช้ binary frame ที่กระชับและมี CRC ตรวจสอบ (Lab 22)
